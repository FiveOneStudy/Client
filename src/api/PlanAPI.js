...
export const updateMonth = async (planId, newContent) => {
  const res = await fetch(`${BASE}/month/update`, {
    method: 'PUT',
    headers: headers(),
    body: JSON.stringify({ planId, newContent }),
  });
  return res.json();
};

export const deleteMonth = async (planId) => {
...
export const modifyCheck = async (checkId, newContent) => {
  const res = await fetch(`${BASE}/check/update`, {
    method: 'PUT',
    headers: headers(),
    body: JSON.stringify({ checkId, newContent }),
  });
  return res.json();
};

export const completeCheck = async (checkId) => {
